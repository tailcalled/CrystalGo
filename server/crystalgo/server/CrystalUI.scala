...
class CrystalUI extends Application {
  ...
  def makeMenu(showGameTab: (CrystalServer, String) => Unit) = {
    ...
    GridPane.setConstraints(titleBox, 4, 0)
    ...
    GridPane.setConstraints(sizeBox, 4, 1)
    ...
    GridPane.setConstraints(komiLabel, 0, 2)
    GridPane.setConstraints(komiBox, 4, 2)
    GridPane.setConstraints(komiLabel2, 5, 2)
    ...
    GridPane.setConstraints(portBox, 4, 3)
    ...
    GridPane.setConstraints(modulesLabel, 14, 0)
    GridPane.setConstraints(chatModuleBox, 14, 1)
    GridPane.setConstraints(namesModuleBox, 14, 2)
    ...
    GridPane.setConstraints(hostButton, 14, 3)
  ...
  def makeGame(server: CrystalServer) = {
    ...
    server.install("crystalui", new CrystalModule {
      ...
      def installHooks(mirror: Mirror) = {
        ...
        mirror.recieveHook((client, id, pro) => events.appendText(pro.toString + "\n"))
      }
